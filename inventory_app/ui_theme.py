...
# Light theme palette (Litera-inspired)
COLOR_APP_BG = "#F8F9FA"        # Light background
COLOR_CARD_BG = "#FFFFFF"       # White card background
COLOR_PRIMARY = "#0D6EFD"       # Blue primary
COLOR_SUCCESS = "#198754"       # Green success
COLOR_DANGER = "#DC3545"        # Red danger
COLOR_WARNING = "#FFC107"       # Yellow warning
COLOR_INFO = "#0DCAF0"          # Cyan info
COLOR_TEXT_MAIN = "#212529"     # Dark gray text
COLOR_TEXT_MUTED = "#6C757D"    # Muted gray text
...
def setup_theme(root=None, theme_name="litera"):
    ...
    try:
        ...
        style.configure("TEntry", font=DEFAULT_FONT, padding=12, fieldbackground=COLOR_CARD_BG, foreground=COLOR_TEXT_MAIN, borderwidth=1)
        
        # Premium Buttons
        style.configure("TButton", font=FONT_BOLD, padding=12, borderwidth=0, borderradius=10) # Modern feel

        style.configure("Primary.TButton", background=COLOR_PRIMARY, foreground="#000000") # High contrast blue
        style.map("Primary.TButton", background=[('active', '#00ADDB'), ('pressed', '#0083A6')], foreground=[('!disabled', '#000000')])

        style.configure("Success.TButton", background=COLOR_SUCCESS, foreground="#000000")
        style.map("Success.TButton", background=[('active', '#00DB75'), ('pressed', '#00A658')], foreground=[('!disabled', '#000000')])

        style.configure("Danger.TButton", background=COLOR_DANGER, foreground="#ffffff")
        style.map("Danger.TButton", background=[('active', '#DB3535'), ('pressed', '#A62828')], foreground=[('!disabled', '#ffffff')])

        style.configure("Secondary.TButton", background=COLOR_BORDER, foreground=COLOR_TEXT_MAIN)
        style.map("Secondary.TButton", background=[('active', '#2C3E50'), ('pressed', '#1A252F')], foreground=[('!disabled', '#ffffff')])

        # Treeview (Light/Clean)
        style.configure("Treeview", font=DEFAULT_FONT, rowheight=30, background=COLOR_CARD_BG, fieldbackground=COLOR_CARD_BG, foreground=COLOR_TEXT_MAIN)
...
def make_card(master, **kwargs):
    ...
    if USING_BOOTSTRAP and tb is not None:
        # Use default background for cards

...
class DynamicFormBuilder:
    ...
    def __init__(self, parent, industry_type: str, form_type: str = 'products'):
        ...
        self.fields = {}
        self.widgets = {}
        self._boolean_vars = {}  # Track QCheckBox states
    ...
    def _build_form(self, container: Optional = None) -> Dict:
        """Build form using PySide6 widgets."""
        from ui_theme import styled_label
        from database import get_db_cursor

        parent = container if container else self.parent
        # If parent is a QWidget, create a layout; if it already has a layout, use it
        ...
        for field in self.field_configs:
            ...
            if field['type'] == 'select':
                ...
                if field.get('default'):
                    idx = widget.findText(str(field['default']))
                    if idx >= 0:
                        widget.setCurrentIndex(idx)

            elif field['type'] == 'boolean':
                widget = QtWidgets.QCheckBox()
                self._boolean_vars[field['name']] = widget
                if field.get('default'):
                ...
                validator = QtGui.QIntValidator() if isinstance(field.get('default'), int) else QtGui.QDoubleValidator()
                ...
                widget = QtWidgets.QLineEdit()
                if field.get('default'):
                    widget.insert(str(field['default']))
    ...
    def set_values(self, values: Dict[str, Any]):
        ...
        for name, value in values.items():
            if name in self.widgets:
                ...
                # Handle boolean fields via QCheckBox
                if name in self._boolean_vars:

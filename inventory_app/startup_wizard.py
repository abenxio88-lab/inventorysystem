...
def create_startup_wizard(parent, on_complete_callback=None):
    ...
    wizard.title("Core Platform Intelligence - Initial Setup")
    wizard.geometry("960x780")
    wizard.resizable(False, False)
    wizard.configure(bg=COLOR_APP_BG)
    ...
    def show_step(step_num):
        ...
        progress_var.set(f"PHASE {step_num}: SYSTEM CALIBRATION" if step_num > 1 else "PHASE 1: SYSTEM IDENTIFICATION")
    ...
    def show_welcome():
        ...
        styled_label(header_frame, text="✨ INITIALIZING PLATFORM", font=FONT_HEADING, foreground=COLOR_PRIMARY).pack(anchor=tk.W)
        styled_label(header_frame, 
                    text="Configure your enterprise environment parameters",
                    foreground=COLOR_TEXT_MUTED).pack(anchor=tk.W, pady=(5, 0))
    ...
    def show_company_type():
        ...
        styled_label(header_frame, text="🏢 SELECT BUSINESS PROFILE", font=FONT_HEADING, foreground=COLOR_PRIMARY).pack(anchor=tk.W)
        styled_label(header_frame, 
    ...
    def show_features():
        ...
        styled_label(header_frame, text="✅ SYSTEM CONFIGURATION OVERVIEW", font=FONT_HEADING, foreground=COLOR_SUCCESS).pack(anchor=tk.W)
        styled_label(header_frame, 

    ...
    def refresh_orders():
        ...
        with get_db_cursor() as cur:
            if status_filter == 'all':
                cur.execute("""
                    SELECT so.id, so.order_number, so.customer_name, so.order_date, so.delivery_date,
                           so.status, so.total_amount, so.payment_status,
        ...
        try:
            with get_db_cursor() as cur:
                ...
                cur.execute("""
                    ...
                    (order_number, customer_name, customer_phone, customer_email, order_date, delivery_date, 
                     status, payment_status, paid_amount, notes, created_by)
                    VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, ?, 'confirmed', ?, ?, ?, ?)
                """, (order_number, customer_name or 'Walk-in', customer_phone, customer_email, 
                      delivery_date or None, payment_status, 
                      float(paid_amount) if paid_amount else 0, notes, current_user))
                ...
                for item in order_items:
                    cur.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?)
                    """, (order_id, item['product_id'], item['quantity'], item['unit_price'], item['total']))
            ...
            # Refresh parent
            if hasattr(parent, 'refresh_orders'):
...
def open_order_details(parent, order_id, current_user=None):
    ...
    with get_db_cursor() as cur:
        ...
        cur.execute("""
            SELECT p.model, soi.quantity, soi.unit_price, soi.total_price

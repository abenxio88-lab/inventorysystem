            ...
            if forecast["stockout_risk"] in ["critical", "high", "medium"]:
                ...
                cursor.execute("""
                    SELECT p.supplier_id, s.name as supplier_name, 
                           p.cost_price, p.min_stock_level
                    FROM products p
...
class DynamicPricingAdvisor:
    ...
    def get_aging_inventory(self, days_threshold: int = 90) -> List[Dict]:
        ...
        query = """
            SELECT p.id, p.name, p.current_stock, p.sell_price, p.cost_price,
                   MIN(si.sale_date) as last_sale_date,
                   julianday('now') - julianday(COALESCE(MIN(si.sale_date), ?)) as days_since_sale
            FROM products p
    ...
    def get_optimal_pricing(self, product_id: int) -> Dict[str, Any]:
        ...
        # Get product info
        cursor.execute("""
            SELECT id, name, current_stock, sell_price, cost_price, category
        ...
        # Get sales velocity
        cursor.execute("""
            SELECT COUNT(*) as total_sales, SUM(quantity) as total_qty,
                   AVG(unit_price) as avg_price
            FROM sales_items
            WHERE product_id = ? AND sale_date >= date('now', '-90 days')
        """, (product_id,))
...
class InventoryHealthAnalyzer:
    ...
    def get_health_score(self) -> Dict[str, Any]:
        ...
        cursor.execute("SELECT SUM(current_stock * cost_price) FROM products")
        total_value = cursor.fetchone()[0] or 0
        
        cursor.execute("SELECT SUM(current_stock * sell_price) FROM products")
